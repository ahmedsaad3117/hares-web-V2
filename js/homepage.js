...
async function initializeHomepage() {
    // If already authenticated, redirect immediately (backup for the head script)
    if (typeof isAuthenticated === 'function' && isAuthenticated()) {
        const user = typeof getCurrentUser === 'function' ? getCurrentUser() : null;
        if (user && user.userId) {
            const isExpired = user.roleName !== 'Super Admin' &&
                typeof isSubscriptionExpired === 'function' &&
                isSubscriptionExpired(user.expirationDate);
            window.location.replace(isExpired ? 'pages/my-subscription.html' : 'pages/dashboard.html');
            return;
        }
    }
...
function toggleSubscribePassword() {
    const input = document.getElementById('subAdminPassword');
    const icon = document.getElementById('subscribeEyeIcon');
